...
export function RecipeCardGrid({recipeCards}: Readonly<RecipeCardGridProps>): React.JSX.Element {
    return (
        <div className="cards">
            {
                recipeCards.map(card => (
                    <Link className='card-link' key={card.recipeId} to={card.recipeLink}>
                        <div className='card'>
                            ...
                            {card.relevantIngredients.map((line, index) => <p
                                key={`${card.recipeId}-${index}`}>{line}</p>)}
                        </div>
                    </Link>
                ))
            }
