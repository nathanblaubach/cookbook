...
export class RecipeRepository implements RecipeRepository {
  ...
  public getRecipesBySearchTermAndCategories(
    searchTerm: string,
    categories: string[],
    includeIngredientMatches: boolean,
  ): Recipe[] {
    return this.recipeReader.readRecipes().filter((recipe) => {
      const recipeNameMatched = this.includesCaseInsensitive(
        recipe.name,
        searchTerm,
      );

      const recipeIngredientMatched =
        includeIngredientMatches &&
        recipe.ingredients.some((ingredient: string) =>
          this.includesCaseInsensitive(ingredient, searchTerm),
        );

      const categoryMatched =
        categories.length === 0 || categories.includes(recipe.category);
